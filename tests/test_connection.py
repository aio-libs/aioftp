...
@pytest.mark.asyncio
async def test_pasv_connection_ports(pair_factory, Server, unused_tcp_port_factory):
...
@pytest.mark.asyncio
async def test_pasv_connection_port_reused(pair_factory, Server, unused_tcp_port):
    ...
    async with pair_factory(None, s) as pair:
        ...
        pair.client.close()
        assert pair.server.available_data_ports.qsize() == 1
        await pair.client.connect(pair.server.server_host, pair.server.server_port)
...
@pytest.mark.asyncio
async def test_pasv_connection_pasv_forced_response_address(pair_factory, Server, unused_tcp_port):
    def ipv4_used():
        try:
...
@pytest.mark.parametrize("method", ["epsv", "pasv"])
@pytest.mark.asyncio
async def test_pasv_connection_no_free_port(pair_factory, Server, expect_codes_in_exception, method):
    s = Server(data_ports=[])
...
@pytest.mark.asyncio
async def test_pasv_connection_busy_port(pair_factory, Server, unused_tcp_port_factory):
    ports = [unused_tcp_port_factory(), unused_tcp_port_factory()]
    async with pair_factory(None, Server(data_ports=ports)) as pair:
...
@pytest.mark.asyncio
async def test_pasv_connection_busy_port2(pair_factory, Server, unused_tcp_port_factory, expect_codes_in_exception):
...
async def test_long_login_sequence_fail(pair_factory, expect_codes_in_exception):
    ...
    factory = pair_factory(logged=False, server_factory=CustomServer, do_quit=False)
