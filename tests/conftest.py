    ...
    class Factory:
        ...
        async def make_client_files(self, *paths, size=None, atom=b"-"):
            ...
            for p in map(Path, paths):
                await self.client.path_io.mkdir(p.parent, parents=True, exist_ok=True)
        ...
        async def __aenter__(self):
            ...
            if self.connected:
                await self.client.connect(self.server.server_host, self.server.server_port)
                if self.logged:
...
@pytest.fixture(params=[aioftp.MemoryPathIO, aioftp.PathIO, aioftp.AsyncPathIO])
def path_io(request):
...
class Sleep:
    ...
    def is_close(self, delay, *, rel_tol=0.05, abs_tol=0.5):
        ...
        if not ok:
            print(f"latest sleep: {self.delay}; expected delay: " f"{delay}; rel: {rel_tol}")
        return ok
...
@pytest_asyncio.fixture(params=[("127.0.0.1", socket.AF_INET), ("::1", socket.AF_INET6)])
async def socks(request, unused_tcp_port):
    ...
    server = await asyncio.start_server(handler, host=host, port=port, family=family)
