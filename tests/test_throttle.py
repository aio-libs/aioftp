...
@pytest.mark.asyncio
async def test_client_side_throttle(pair_factory, skip_sleep, times, type, direction):
...
@pytest.mark.parametrize("throttle_level", ["throttle", "throttle_per_connection"])
@pytest.mark.asyncio
async def test_server_side_throttle(
    ...
    async with pair_factory() as pair:
        ...
        throttle = reduce(getattr, [throttle_level, throttle_direction], pair.server)
